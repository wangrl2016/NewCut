...
namespace base {
    const wchar_t* const kConvertRoundtripCases[]{
            // 在Windows中L占用两个字节，在POSIX中L占用4个字节
            L"Google Video",
            // "网页 图片 资讯更多 »"
            ...
            // 𐌀
            L"\x10300",
            // 𑵀𑵁𑵂𑵃𑵄 (Mathematical Alphanumeric Symbols (U+011d40 - U+011d44 : A,B,C,D,E)
    ...
    TEST(UTFStringConversionsTest, ConvertUTF8AndWideEmptyString) {
        ...
        std::wstring w_empty;
        std::string empty;
        EXPECT_EQ(empty, WideToUTF8(w_empty));
        EXPECT_EQ(w_empty, UTF8ToWide(empty));
    }
    ...
    TEST(UTFStringConversionsTest, ConvertUTF8ToWide) {
        ...
        // Manually test an embedded nullptr.
        std::wstring converted;
